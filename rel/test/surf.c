...
inline void rho_grad(double *point, double *rho, double *grad, double *gradmod){
  ...
  *gradmod = 0.0;
  ...
  for (i=0; i<nmo_; i++){
    ...
    c3_[i] = 0.0;
    for (j=0; j<nprims_; j++){
      c0_[i] += (ao_[0*nprims_+j])*mo_coeff_[i*nprims_+j];
  ...
  for (i=0; i<nmo_; i++){
    ...
    c3_[i] = 0.0;
    for (j=0; j<nprims_; j++){
      c0_[i] += (ao_[4*nprims_+0*nprims_+j])*mo_coeff_[i*nprims_+j];
  ...
  for (i=0; i<nmo_; i++){
    ...
    for (j=0; j<nprims_; j++){
      c0_[i] += (ao_[0*nprims_+j])*mo_coeff_[i*nprims_+j];
      c1_[i] += (ao_[4*nprims_+0*nprims_+j])*mo_coeff_[i*nprims_+j];
		}
	}
  ...
  mx = creal(rab) + creal(rba);
  my = cimag(rba) - cimag(rab);
