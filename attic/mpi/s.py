...
# don't modify the following attributes, they are not input options
    ...
    def surface(self):
        ...
        if (rank == 0): 
            idx = numpy.arange(self.npang, dtype=numpy.int32)
        else:
            idx = numpy.empty(self.npang, dtype=numpy.int32)

        lsize = self.npang/size
        idxl = numpy.empty(lsize, dtype=numpy.int32)
        nlimsurf = numpy.empty(lsize, dtype=numpy.int32)
        rsurf = numpy.empty((lsize,self.ntrial))
        comm.Scatter([idx, MPI.INT], [idxl, MPI.INT])

        for i in range(lsize):
            # check index if adjusted size to avoid padding points
            ind = idxl[i]
            ncount = 0
            nintersec = 0
            cost = self.grids[ind,0]
            sintcosp = self.grids[ind,1]*self.grids[ind,2]
            sintsinp = self.grids[ind,1]*self.grids[ind,3]
            ia = self.inuc
            ra = 0.0
            for j in range(self.ntrial):
                ...
                ier, xpoint = self.odeint(xpoint,self.step)
            ...
            # organize pairs
            nlimsurf[i] = nintersec
            for ii in range(nintersec):
                rsurf[i,ii] = xsurf[ii,2]
            if (nintersec%2 == 0):
                nintersec = +1
                nlimsurf[i] += nintersec
                rsurf[i,nintersec-1] = self.rmaxsurf
            print("#* ",rank,ind,self.grids[ind,:4],rsurf[i,:nintersec], nlimsurf[i])

        # Collect info
        comm.Gather([nlimsurf, MPI.INT], [self.nlimsurf, MPI.INT])
        comm.Gather([rsurf, MPI.DOUBLE], [self.rsurf, MPI.DOUBLE])
