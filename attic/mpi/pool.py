...
# don't modify the following attributes, they are not input options
    ...
    def dump_input(self):
        ...
        logger.info(self,'Chkfile %s' % self.chkfile)
        ...
        logger.info(self,'Nuc position %8.5f %8.5f %8.5f', *self.xnuc)
    ...
    def build(self):
        ...
        if (rank == 0):
            ...
            if (gradmod > 1e-4):
                ...
                logger.info(self,'Check rho position %8.5f %8.5f %8.5f', *self.xyzrho)
        self.xyzrho = comm.bcast(self.xyzrho, root=0)
    ...
    def surface(self):
        ...
        def ray(task):
            ...
            xsurf = numpy.zeros((x.ntrial,3))
            isurf = numpy.zeros((x.ntrial,2), dtype=numpy.int32)
            rsurf = numpy.zeros((x.ntrial))

            ind = y
            ncount = 0
            nintersec = 0
            cost = x.grids[ind,0]
            sintcosp = x.grids[ind,1]*x.grids[ind,2]
            sintsinp = x.grids[ind,1]*x.grids[ind,3]
            ia = x.inuc
            ra = 0.0
            for j in range(x.ntrial):
                ract = x.rpru[j]
                xdeltain[0] = ract*sintcosp
                xdeltain[1] = ract*sintsinp
                xdeltain[2] = ract*cost    
                xpoint = x.xnuc + xdeltain
                #ier, xpoint = x.odeint(xpoint,x.step)
                ier, xpoint, rho, gradmod = x.odeint(xpoint,x.step)
                #rho, grad, gradmod = x.rhograd(xpoint)
                good, ib = x.checkcp(xpoint,rho,gradmod)
                rb = ract
                if (ib != ia and (ia == x.inuc or ib == x.inuc)):
                    if (ia != x.inuc or ib != -1):
                        nintersec += 1
            ...
            for k in range(nintersec):
                ...
                ra = xsurf[k,0]
                rb = xsurf[k,1]
                xin[0] = x.xnuc[0] + ra*sintcosp
                xin[1] = x.xnuc[1] + ra*sintsinp
                xin[2] = x.xnuc[2] + ra*cost
                xfin[0] = x.xnuc[0] + rb*sintcosp
                xfin[1] = x.xnuc[1] + rb*sintsinp
                xfin[2] = x.xnuc[2] + rb*cost
                while (abs(ra-rb) > x.epsroot):
                    xmed = 0.5*(xfin+xin)    
                    rm = 0.5*(ra+rb)
                    xpoint = xmed
                    #ier, xpoint = x.odeint(xpoint,x.step)
                    ier, xpoint, rho, gradmod = x.odeint(xpoint,x.step)
                    #rho, grad, gradmod = x.rhograd(xpoint)
                    good, im = x.checkcp(xpoint,rho,gradmod)
            ...
            if (nintersec%2 == 0):
                nintersec = +1
                rsurf[nintersec-1] = x.rmaxsurf
            #print("#* ",rank,ind,self.grids[ind,:4],rsurf[:nintersec], nintersec)
            return ind, nintersec, rsurf
        ...
        # Pass self object and idx of ray
        info = []
        idx = []
        for i in range(self.npang):
            info.append(self)
            idx.append(i)
        tasks = numpy.vstack((info,idx)).T
        val = pool.map(ray, tasks)
