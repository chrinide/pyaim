...
if __name__ == '__main__':
    ...
    a = numpy.asarray(a)
    t0 = time.clock()
    valor, valor1, valor2, valor3, valor4 = density(a)
    log.timer('simple', t0)
    print a, valor, valor1, valor2, valor3, valor4

    mol = lib.chkfile.load_mol('h2o.chk')
    mf = lib.chkfile.load('h2o.chk', 'scf')
    mo_coeff = lib.chkfile.load('h2o.chk', 'scf/mo_coeff')
    mo_occ = lib.chkfile.load('h2o.chk', 'scf/mo_occ')
    a = numpy.reshape(a, (-1,3))
    #t0 = time.clock()
    #for i in range(300):
    #    ao = dft.numint.eval_ao(mol, a, deriv=2)
    #    rho = dft.numint.eval_rho2(mol, ao, mo_coeff, mo_occ, xctype='MGGA')
    #log.timer('complex', t0)
    #print a, rho

    import hess
    t0 = time.clock()
    for i in range(3000):
        rho = hess.eval_rho3(mol, a, mo_coeff, mo_occ, deriv=1)
    log.timer('complex', t0)
    #print a, rho
