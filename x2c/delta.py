...
with open(name+'_'+str(atm[0])+'_1c.molden', 'w') as f1:
    molden.header(mol, f1)
    molden.orbital_coeff(mol, f1, natorb, occ=natocc)
log.info('DAFH for atom %d', atm[1])
natocc, natorb = numpy.linalg.eigh(aom2)
natocc *= 2.0
log.info('Occ for DAFH %s', natocc)
log.info('Sum Occ for DAFH %f', natocc.sum())
natorb = numpy.dot(mo_coeff, natorb)
with open(name+'_'+str(atm[1])+'_1c.molden', 'w') as f1:
    molden.header(mol, f1)
    molden.orbital_coeff(mol, f1, natorb, occ=natocc)
